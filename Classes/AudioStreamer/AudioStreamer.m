...
- (id)initWithURL:(NSURL *)aURL andMediaType:(NSString *)aMediaType
{
	self = [self initWithURL:aURL];
	if (self != nil)
	{
      mediaType = [aMediaType copy];
	}
	return self;
}
...
- (void)startInternal
{
	...
	} while (isRunning && ![self runLoopShouldExit] && !forceStop);
...
//   int count = 0;
//	while (state != AS_INITIALIZED && count < 100)  // Exit loop after 10 seconds regardless of state. Prevents infinite loop.
//	{
	...
	eventType:(CFStreamEventType)eventType
{
	...
	else if (eventType == kCFStreamEventHasBytesAvailable)
	{
		...
		if (!audioFileStream)
		{
			...
			//
         // ### Added mediaType checked by Raizlabs. ###
			AudioFileTypeID fileTypeHint;
         if([mediaType isEqualToString:@"audio/mpeg"])
         {
            fileTypeHint = kAudioFileMP3Type;
         }
         else if([mediaType isEqualToString:@"audio/aacp"])
         {
            fileTypeHint = kAudioFileAAC_ADTSType;
         } else {
            ...
            if (foundIcyStart && !foundIcyEnd)
            {
               ...
               while (YES)
               {
                  ...
                  {		
                     ...
                     // KT: Modified to handle unicode characters.
                     NSInteger rangeLength = (streamStart-lineStart)-(streamStart-[fullString length]);
                     NSString *line = [fullString substringWithRange:NSMakeRange(lineStart, rangeLength)];
			...
			(state == AS_STOPPED && stopReason == AS_STOPPING_TEMPORARILY))
		{
			...
			if (state == AS_FLUSHING_EOF || buffersUsed == kNumAQBufs - 1)
			{
               ...
               @catch (NSException *exception) {
                  DLog(@"AudioQueueStart exception: %@ %@ %@", [exception name], [exception reason], [exception userInfo]);
                  [self failWithErrorCode:AS_AUDIO_QUEUE_START_FAILED];
                  return;
               }
				}
...
- (void)createQueue
{
	...
	// Needed for level metering.
	numberOfChannels = asbd.mChannelsPerFrame;
   // End level metering change.
